...
export default function NotificationBell() {
  ...
  const getNotificationIcon = (notification: any) => {
    // Check metadata.eventType first, then fall back to type
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative p-2 text-gray-400 hover:text-orange-400 focus:outline-none transition-colors"
      >
        <svg
          className="w-6 h-6"
          fill="none"
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-gray-900 rounded-xl shadow-2xl z-[9999] border border-gray-700">
              ...
              notifications.map((notification, index) => (
                ...
                >
                  <div className="flex items-start">
                    <span className="text-2xl mr-3">{getNotificationIcon(notification)}</span>
